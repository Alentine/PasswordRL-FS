...
class PRLModel(nn.Module):
    ...
    def forward_train(self, pwds, edits, autoregressive_weight=0.1):
        ...
        for t in range(maxlen):
            ...
            outputs[:, t] = decoder_output.squeeze(1)   # log softmax
            decoder_input = edits[:, t].unsqueeze(1)    # teacher forcing

        if autoregressive_weight > 0:
            with torch.no_grad():
                decoder_hidden_auto = encoder_hidden
                decoder_input_auto = self.decoder.init_start_token(batch_size, self.start_token, self.device)
                outputs_auto = torch.zeros(batch_size, maxlen, self.output_size).to(self.device)

                for t in range(maxlen):
                    decoder_output_auto, decoder_hidden_auto = self.decoder(decoder_input_auto, decoder_hidden_auto)
                    outputs_auto[:, t] = decoder_output_auto.squeeze(1)   # log softmax
                    # 贪婪选择下一个 token
                    top1 = decoder_output_auto.squeeze(1).argmax(dim=1)   # (batch,)
                    decoder_input_auto = top1.unsqueeze(1)                # (batch, 1)

            outputs = (1 - autoregressive_weight) * outputs + autoregressive_weight * outputs_auto
